...
def TrainOperation(TrainLabels, NumTrainSamples, ImageSize,
    ...
    # scheduler = torch.optim.lr_scheduler.StepLR(Optimizer, step_size=10, gamma=0.1)  # Reduce LR every 10 epochs by factor of 0.1
    ...
    for Epochs in tqdm(range(StartEpoch, NumEpochs)):
        ...
        for PerEpochCounter in tqdm(range(NumIterationsPerEpoch)):
            ...
            train_Batch = GenerateBatch(TrainSet, TrainLabels, ImageSize, len(TrainSet)//1000)
            result = model.validation_step(train_Batch)
...
def main():
    ...
    CheckPointPath = 'dtrivedi_hw0/Phase2/Code/Checkpoints/'
